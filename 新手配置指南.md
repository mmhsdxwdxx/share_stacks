...
|------|---------|---------|---------|--------|
| 1 | `.env` | 项目根目录 | ⭐⭐ | ✅ 必需 |
| 2 | `mcpo/config.template.json` | mcpo/ | ⭐⭐⭐ | ✅ 必需 |
...
🔴 高优先级（必须配置）
├─ .env 文件中的所有密码和密钥
├─ .env 中的 LITELLM_MCP_VKEY（mcpo 访问 LiteLLM 的 Virtual Key）
...
### 配置文件：`mcpo/config.template.json`

**文件路径**: `share_stacks/mcpo/config.template.json`
**配置方式**: 文本编辑器
**安全级别**: 🟡 中度敏感
**配置时机**: 启动服务前/后均可（修改后需重启 mcpo）
...
**文件路径**: `mcpo/config.template.json`
...
### 步骤 2.3: 配置 LiteLLM Virtual Key（mcpo 访问 LiteLLM）

**重要**: `mcpo/config.template.json` 中的 `__LITELLM_MCP_VKEY__` 是占位符；真实密钥通过 `.env` 的 `LITELLM_MCP_VKEY` 在容器启动时自动注入到 `/config/config.json`。

**推荐配置方式**：

在 `.env` 文件中填写：
```bash
# LiteLLM Virtual Key for mcpo（在 LiteLLM UI 创建，切勿使用 Master Key）
LITELLM_MCP_VKEY=sk-YourLitellmVirtualKeyHere
```
...
编辑 `mcpo/config.template.json`（保留 `__LITELLM_MCP_VKEY__` 占位符）:
```json
{
  "mcpServers": {
    "deepwiki": {
...
一般情况下无需修改配置文件：`mcpo/config.template.json` 内已包含 `__LITELLM_MCP_VKEY__` 占位符，容器启动时会自动注入到 `/config/config.json`。

如需新增/修改 MCP 列表，请编辑 `mcpo/config.template.json`（保留 `__LITELLM_MCP_VKEY__` 占位符）：
```json
{
  "mcpServers": {
    "deepwiki": {
...
share_newapi         calciumion/new-api:v0.10.4        Up
share_litellm        share-stacks/litellm:custom       Up
share_mcpo           ghcr.io/open-webui/mcpo:git-25b219a@sha256:88e4c770265810352cb420b2315391b751fafd96f8989cd37dce7b81631e6d1e  Up
```
...
**解决方案**:
```bash
# 检查 .env 中的 Virtual Key（是否填错/多空格）
grep "^LITELLM_MCP_VKEY=" .env

# 查看 mcpo 实际运行时配置（容器内由模板生成）
docker compose exec mcpo cat /config/config.json | head -n 60
...
├── mcpo/
│   └── config.template.json     # MCP转换器配置模板（运行时生成 /config/config.json）
├── postgres-init/
...
cat mcpo/config.template.json

# 编辑配置
nano .env
nano litellm/config.yaml
nano mcpo/config.template.json
```
...
| MCP配置 | `mcpo/config.template.json` | `nano mcpo/config.template.json` | `docker compose restart mcpo` |
...
第13分钟  配置 `.env` 的 LITELLM_MCP_VKEY（必要时修改 mcpo/config.template.json）
第14分钟  重启 mcpo 服务
第15分钟  验证所有功能
```
