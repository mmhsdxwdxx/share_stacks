...
4. 将 Virtual Key 写入 `.env`（推荐做法，不要把密钥写死在配置文件里）:
```bash
nano .env
```
...
说明：
- `mcpo/config.template.json` 内部使用占位符 `__LITELLM_MCP_VKEY__`
- 容器启动时会自动从模板生成 `/config/config.json` 并注入该 Key
...
4. **添加更多 MCP**: 编辑 `mcpo/config.template.json`（保留 `__LITELLM_MCP_VKEY__` 占位符）
